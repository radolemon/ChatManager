...
public class ChatLog {
    ...
    public static void logMessage(String serverName, String rawMessage) {
        ...
        if (!serverDir.exists()) {
            if (serverDir.mkdirs()) {
                ChatManagerClient.LOGGER.info("Created chat log directory for server: {}", serverName);
            }
        }
        ...
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(ChatManagerClient.LOG_FILE, true))) {
            ...
            ChatManagerClient.LOGGER.error("Failed to write chat log to {}", ChatManagerClient.LOG_FILE.getName(), e);
        }
    }

    // （オプション）メッセージからプレイヤー名を抜き出す処理を残しておきたい場合
    ...
    public static void cleanupOldLogs() {
        ...
        for (int i = 0; i < filesToDelete; i++) {
            ...
            if (file.delete()) {
                ChatManagerClient.LOGGER.info("Deleted old chat log: {}", file.getName());
            } else {
                ChatManagerClient.LOGGER.warn("Failed to delete chat log: {}", file.getName());
            }
        }
    }
}
