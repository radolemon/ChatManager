...
public class ConfigLoader {
    ...
    public static void load() {
        ...
        if (!CONFIG_FILE.exists()) {
            save(); // 初回にファイル作成
            ChatManagerClient.LOGGER.info("Config file not found, created default at: {}", CONFIG_FILE.getAbsolutePath());
        }

        try (InputStream input = new FileInputStream(CONFIG_FILE)) {
            props.load(input);
            ChatManagerClient.LOGGER.info("Loaded config.properties from {}", CONFIG_FILE.getAbsolutePath());
        } catch (IOException e) {
            ChatManagerClient.LOGGER.error("Failed to load config.properties", e);
        }
        ...
        try {
            ...
            ChatManagerClient.LOGGER.warn("Invalid number for maxLogFilesPerServer, using default: {}", MaxLogFilesPerServer);
        }
    }
    ...
    public static void save() {
        ...
        try {
            ...
            ChatManagerClient.LOGGER.info("Saved config.properties");
        } catch (IOException e) {
            ChatManagerClient.LOGGER.error("Failed to save config.properties", e);
        }
    }
